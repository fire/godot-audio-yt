...
bool _fetch_player_response(const String p_id, yt::PlayerResponse &r_response, const bool *p_terminate_threads) {
	...
	Variant player_data;
	{
		...
		const Error json_error = json->parse(player_data_raw);
		if (json_error != OK) {
			print_line("");
			print_line("");
			print_line("");
	...
	{
		Ref<JSON> json = memnew(JSON);
		const Error json_error = json->parse(player_response_raw);
		if (json_error != OK) {
			Ref<core_bind::File> file;
			file.instantiate();
			file->open("res://response.html", core_bind::File::WRITE);
			...
			print_line("DATA:");
			print_line("");
			print_line(player_response_raw);
			print_line("");
			ERR_PRINT("Failed to parse player response json.");
			return false;
		}
		player_response = json->get_data();
	}
...
String yt::YouTube::request(
	...
	if (!p_body.is_empty()) {
		const Error request_err = client->request(HTTPClient::METHOD_POST, p_host + p_path, p_headers, (const uint8_t *)p_body.utf8().get_data(), p_body.utf8().length());
		ERR_FAIL_COND_V_MSG(request_err != OK, "", "Failed to perform request.");
	} else {
		PackedByteArray body;
		const Error request_err = client->request(HTTPClient::METHOD_GET, p_host + p_path, p_headers, body.ptrw(), body.size());
		ERR_FAIL_COND_V_MSG(request_err != OK, "", "Failed to perform request.");
	}
	...
	if (client->has_response()) {
		...
		if (p_file.is_empty()) {
			...
			String text;
			text.parse_utf8((const char *)response.ptrw(), response.size());

			return text;
		} else {
			// Create directory.
			Ref<DirAccess> dir = DirAccess::create_for_path(p_file);
			...
			Error file_err;
			Ref<FileAccess> file = FileAccess::open(tmp_file, FileAccess::WRITE, &file_err);
...
void yt::YouTube::_thread_search(Ref<YouTubeSearchTask> p_task) {
	...
	const String body = JSON::stringify(get_body());
	...
	Variant search_response;
	{
		...
		int err_line;
		Ref<JSON> json = memnew(JSON);
		const Error json_error = json->parse(search_response_raw);
		ERR_FAIL_COND_MSG(json_error != OK, "Failed to parse player response json.");
		search_response = json->get_data();
	}
	...
	auto parse_duration = [&](const Variant p_duration) -> double {
		...
		for (int i = 0; i < nums.size(); ++i) {
			const double v = nums[i].to_float();
			switch (i) {
				case 0: {
					duration += v;
				} break;
	...
	for (int i = 0; i < sections.size(); ++i) {
		...
		for (int j = 0; j < contents.size(); ++j) {
			...
			result->create(
					...
					try_runs(item.get("viewCountText")).to_int(),
					from_artist);
...
void yt::YouTube::_thread_get_video(Ref<YouTubeGetVideoTask> p_task) {
	...
	result->create(
			...
			details.get("author"),
			details.get("title"),
			String(details.get("lengthSeconds")).to_int(),
			String(details.get("viewCount")).to_int(),
			from_artist);
...
void yt::YouTube::download_cache(const String p_playback_url, const String p_local_path) {
	...
	static HashSet<String> downloading_cache;
...
void yt::Player::_thread_func() {
	auto fetch_scrambler_funcs = [&](const PlayerResponse &p_player, std::vector<ScramblerFunction> &r_scrambler) {
		...
		ERR_FAIL_COND_MSG(player_script.is_empty(), "Player script is empty.");
		...
		if (!valid) {
			auto get_cipher_data = [&]() -> Dictionary {
				...
				for (int i = 0; i < elements.size(); ++i) {
					...
					const String key = pair[0].replace("+", " ").uri_decode();
					const String value = pair[1].replace("+", " ").uri_decode();
					dict[key] = value;
				}
			...
			auto decipher_url = [&](const std::vector<ScramblerFunction> &p_scrambler) {
				...
				for (size_t i = 0; i < p_scrambler.size(); ++i) {
					...
					case ScramblerFunction::Type::SWAP: {
						...
						const CharProxy<char32_t> temp = signature[0];
					...
					case ScramblerFunction::Type::REVERSE: {
						...
						for (int index = 0; index < size / 2; ++index) {
							...
							const CharProxy<char32_t> temp = signature[index];
				...
				playback_url += vformat("&ratebypass=yes&%s=%s", signature_param, String(signature.get_data()).uri_decode());
			};
